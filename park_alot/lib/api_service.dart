...
class ApiService {
  ...
  static const String baseUrl = "http://192.168.208.216:8080";
...
// Taker Login
  ...
  // Get All Cities
  static Future<Map<String, dynamic>> getAllCities() async {
    final response = await http.get(
    ...
    if (response.statusCode == 200) {
      ...
      // Extracting success, message, and data fields
      return {
        "success": responseBody["success"],
        "message": responseBody["message"],
        "data":
        responseBody["data"], // Assuming data contains the list of cities
      };
    } else {
      // Handling non-200 responses
...
// Get Addresses By City
  static Future<Map<String, dynamic>> getAddressesByCity(
      String cityName) async {
    final response = await http.get(
      Uri.parse("$baseUrl/addresses?cityName=$cityName"),
      headers: {"Content-Type": "application/json"},
    );
    ...
    if (response.statusCode == 200) {
      ...
      return {
        ...
        "message": responseBody["message"],
        "data": responseBody[
        "data"], // Assuming data contains the list of addresses
      };
    } else {
      // Handling non-200 responses
